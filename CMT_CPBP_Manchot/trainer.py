...
class CMTtrainer(BaseTrainer):
    ...
    # directly sample melodies from a checkpoint
    def sampling(self, **kwargs):
        # load model if exists
        self.load_model(kwargs["restore_epoch"], kwargs["load_rhythm"])

        # valid and test epoch and sampling
        with torch.no_grad():

            # just generate a midi file from groundtruth pkl files
            if self.config['gen_groundtruth_mid']:
                print('Generating gt file only')
                self.gen_gt_mid(kwargs["restore_epoch"])

            # Check whether we sample with java, or if we want to activate beam search, or just sample normally
            else:
                if not self.config['rhythm_only']:
                    if self.config['sampling_java']:
                        self._sampling_with_java(kwargs["restore_epoch"], self.config['sampling_java_seed'])
                    elif self.config['beam_width']>0:
                        self._sampling_beam_search(kwargs["restore_epoch"])
                    else:
    ...
    def _sampling(self, epoch):
        ...
        result_dict = model.sampling(prime_rhythm, prime, batch['chord'], epoch,
    ...
    '''
    Function called to sample with beam search activated. See sampling_beam_rhythm or sampling_beam_pitch
    functions from model.py 
    '''
    def _sampling_beam_search(self, epoch):
        self.model.eval()
        loader = self.test_loader
        asset_path = os.path.join(self.asset_path)
        ...
        if isinstance(self.model, torch.nn.DataParallel):
            ...
            model = self.model
        prime_rhythm = batch['rhythm'][:, :self.config["num_prime"]]
        if self.config['beam_mode']=='rhythm':
            print('Beam search on rhythm')
            results = model.sampling_beam_rhythm(prime_rhythm, prime, batch['chord'], epoch,
                                     self.config["topk"], self.config["beam_width"], self.config['attention_map'])
        else:
            print('Beam search on pitch')
            results = model.sampling_beam_pitch(prime_rhythm, prime, batch['chord'], epoch,
                                                 self.config["topk"], self.config["beam_width"],
                                                 self.config['attention_map'])

        for sequence_id in range(len(results)):
            result_key = 'pitch'
            pitch_idx = results[sequence_id][result_key].cpu().numpy()
            ...
            for sample_id in range(pitch_idx.shape[0]):
                logger.info(("Sample %02d : " % sample_id) + str(pitch_idx[sample_id][self.config["num_prime"]:self.config["num_prime"]+20]))
                save_path = os.path.join(asset_path, 'sampling_results', 'epoch_%03d' % epoch,
                                         'epoch%03d_sample%02d_beam%02d.mid' % (epoch, sample_id, sequence_id))
                gt_pitch = batch['pitch'].cpu().numpy()
                gt_chord = batch['chord'][:, :-1].cpu().numpy()
                sample_dict = {'pitch': pitch_idx[sample_id],
                               'rhythm': results[sequence_id]['rhythm'][sample_id].cpu().numpy(),
                               'chord': csc_matrix(gt_chord[sample_id])}
    ...
    '''
    Use JEP (java embedded python) to generate melody. See sampling_with_java function from model.py 
    '''
    def _sampling_with_java(self, epoch, seed):
        self.model.eval()
        loader = self.test_loader
        asset_path = os.path.join(self.asset_path)
        ...
        if isinstance(self.model, torch.nn.DataParallel):
            ...
            model = self.model
        prime_rhythm = batch['rhythm'][:, :self.config["num_prime"]]
        result_dict = model.sampling_with_java(prime_rhythm, prime, batch['chord'], epoch, seed,
                                     self.config["topk"], self.config['attention_map'])
        result_key = 'pitch'
        pitch_idx = result_dict[result_key].cpu().numpy()
    ...
    '''
    Save only groundtruth 
    '''
    def gen_gt_mid(self, epoch):
        self.model.eval()
        loader = self.test_loader
        asset_path = os.path.join(self.asset_path)
        ...
        gt_pitch = batch['pitch'].cpu().numpy()
        no_pitch = np.zeros(gt_pitch.shape)
        gt_chord = batch['chord'][:, :-1].cpu().numpy()
        ...
        for sample_id in range(len(indices)):  
            # save groundtruth
            logger.info(("Groundtruth pkl to mid %02d : " % sample_id) +
                        str(gt_pitch[sample_id, self.config["num_prime"]:self.config["num_prime"] + 20]))
            gt_path = os.path.join(asset_path, 'sampling_results', 'epoch_%03d' % epoch,
                                     'epoch%03d_groundtruth%02d.mid' % (epoch, sample_id))
            gt_dict = {'chord': csc_matrix(gt_chord[sample_id])}

            # check if user wants to save melody as well as chords in groundtruth file 
            if self.config['gen_groundtruth_melody']:
                instruments = pitch_to_midi(gt_pitch[sample_id, :-1], gt_chord[sample_id], model.frame_per_bar, gt_path)
            else:
                instruments = pitch_to_midi([], gt_chord[sample_id], model.frame_per_bar, gt_path)

            print(f'saving gt {sample_id}')
            with open(gt_path.replace('.mid', '.pkl'), 'wb') as f_gt:
                pickle.dump(gt_dict, f_gt)
