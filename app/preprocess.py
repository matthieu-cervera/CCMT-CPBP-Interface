...
'''
PREPROCESS
    Create pkl files from MIDI files (the MIDI file needs to have two parts - melody and chords.)
'''
...
def make_instance_pkl_files(root_dir, midi_dir, num_bars, frame_per_bar, pitch_range=48, shift=False,
    ...
    for midi_file in tqdm(midi_files, desc="Processing"):
        ...
        key_count = len(sorted(glob.glob(os.path.join(dir_name, '*_+0_*.pkl'))))
        if shift:
        ...
        for k in pitch_shift:
            ...
            print(f'pianoroll = {pianoroll.shape[1]} and offset_roll={offset_roll.shape[1]}')
            ...
            for i in range(0, timelen - (instance_len + 1), stride):
                ...
                newfilename = filename.split('.mi')[0]
                pkl_filename = os.path.join(dir_name, newfilename, '%s_%02d_%s_%02d.pkl' % (newfilename, key_count, ps, i // stride))
                
                os.makedirs(os.path.join(dir_name, newfilename), exist_ok=True)
